...
class shopweb {
	...
	function redeem($phone,$voucher_hash){
		...
		if ($voucher_hash == NULL) {
			$message['status'] = "error";
			$message['info'] = "กรุณากรอกลิ้งอั่งเปาให้ถูกต้อง";
		}else{
				...
				if ($codestatus == "VOUCHER_OUT_OF_STOCK") {
					...
					if ($balance->amount_baht == $balance->redeemed_amount_baht) {
						...
						if (isset($_SESSION['username'])) {
							...
							$message['info'] = "สำเร็จคุณเติมเงินเข้าแล้ว เป็นจำนวน ";
						} else {
							$message['status'] = "error";
							$message['info'] = "Dinorrrr!!!";
						}
					}else{
						$message['status'] = "error";
	...
	function add_history_topup($name,$amount,$phone){
		$stmt = $this->db->prepare("INSERT INTO history_topup(user_topup, name_topup, amount_topup) VALUES (:user,:amount,:name)");
